...
"""
This file contains utility functions for loading and preprocessing data from STREETS.
"""
...
def get_bound(sensor_dict, sensor_id):
    ...
    # print(name)
    # 下面这句话来区分是入度还是出度  整个函数就是这一个作用
    # 梳理 24-31 8个节点  因为 东西南北 * 入度出度
    return 'inbound' if 'inbound' in name else 'outbound'


# 输入邻接矩阵 传感器id 传感器字典 k值 是否跳过出度
def load_K_hop_neighborhood(adjacency_matrix,
    ...
    # print(home_sensor_bound) 得到的值有两种  inbound 和 outbound
    neighborhood_dict = {0: {'inbound': [], 'outbound': []}}
    neighborhood_dict[0][home_sensor_bound].append(sensor_id)
    # print(neighborhood_dict)  0: {'inbound': [28], 'outbound': []}}
    neighbors = [sensor_id]
...
def load_traffic_data(traffic_data_path, dates='all'):
    ...
    # 这里各天各相机都不会满足6*24  10分钟1张  肯定有空缺  也会有多余
...
def interpolate_traffic_data(traffic_data, T):
    ...
    min_hour = 5
    max_hour = 23
    # n_samples的计算方法 即216的
    n_samples = int(60 * (max_hour - min_hour) / T)
    # iterate through each date  每一天
    for d in traffic_data:
        date_data = traffic_data[d]
        interpolated_data[d] = {}
        # iterate through each sensor  每个传感器
        for s in date_data:
...
def build_feature_vector(idx, n_samples, date, neighborhood_data, neighborhood, history, use_state, use_time_of_day,
                         max_dict):
...
def load_train_data(dates,
    ...
    for date in dates:
        ...
        for idx in valid_sample_indices:
            ...
            X.append(build_feature_vector(idx, n_samples, date, neighborhood_data, neighborhood, history, use_state,
                                          use_time_of_day, max_dict))
...
def mape(x, y):
    return np.mean(np.abs(x - y) / (y+10^(-6)))
